...
function translateStaticHtml(kind: RouteKind, html: string) {
  const replacements: Record<RouteKind, Array<[string, string]>> = {
    landing: [
      ...
      ["Free 开放实时同步的免费模型池，并额外赞助开放 DeepSeek V4 Flash、DeepSeek V4 Pro；MiniMax M3 通过 /v1/messages 免费开放且不计入月度额度；Plus 开放国产主流模型，后端自动分配模型通道。", "Free users can use the live free model catalog plus sponsored DeepSeek V4 Flash and DeepSeek V4 Pro. MiniMax M3 is additionally free through /v1/messages and does not count against monthly quota. Plus unlocks the full paid model pool with automatic backend routing."],
      ...
      ["V4 Flash / V4 Pro 赞助开放 · Free 与 Plus 都可用", "V4 Flash and V4 Pro are sponsored for both Free and Plus users"],
      ["Free · 走 /v1/messages · 不计入月度额度", "Free · uses /v1/messages · no monthly quota"],
      ["套餐额度", "Plan quota"],
      ...
      ["选择套餐", "Choose a plan"],
      ["Free 可用实时同步的免费模型池，以及赞助开放的 DeepSeek V4 Flash / DeepSeek V4 Pro；MiniMax M3 额外免费开放且不计入月度额度；Plus {{PLUS_MONTHLY_PRICE_TEXT}}，开放主流模型池。", "Free includes the live free model catalog plus sponsored DeepSeek V4 Flash / DeepSeek V4 Pro. MiniMax M3 is additionally free and excluded from monthly quota. Plus {{PLUS_MONTHLY_PRICE_TEXT}} unlocks the mainstream model pool."],
      ["获取 Key", "Get a key"],
    ...
    login: [
      ...
      ["验证邮箱后启用 Free 套餐，每月 500 次请求额度，可调用实时同步的免费模型池，以及赞助开放的 DeepSeek V4 Flash / DeepSeek V4 Pro；MiniMax M3 额外免费开放，需走 /v1/messages，且不计入月度额度。", "Verify your email to activate the Free plan: 500 monthly requests, the live free model catalog, sponsored DeepSeek V4 Flash / DeepSeek V4 Pro, plus MiniMax M3 through /v1/messages without monthly quota impact."],
      ["已提交", "Submitted"],
    ],
    models: [
      ["在一个广场里挑模型，在一套 API 里调用。", "Choose models in one catalog and call them through one API."],
      ["OpenAchieve 将实时同步的免费模型池与 Kimi、GLM、MiMo、Qwen、DeepSeek 等 Plus 模型收束到统一域名下。Free 可用免费模型池，以及赞助开放的 DeepSeek V4 Flash、DeepSeek V4 Pro；MiniMax M3 通过 /v1/messages 免费开放且不计入月度额度；Plus 模型统一自动分配通道。", "OpenAchieve brings the live free model catalog, sponsored DeepSeek V4 Flash / DeepSeek V4 Pro, and MiniMax M3 through /v1/messages under one domain, alongside Kimi, GLM, MiMo, Qwen, and other Plus models."],
      ["浏览模型", "Browse models"],
      ...
      ["Free 每月 {{FREE_MONTHLY_REQUESTS}} 次，可调用 <span data-live-free-count>实时免费模型池</span>，包含赞助开放的 DeepSeek V4 Flash 与 DeepSeek V4 Pro；MiniMax M3 通过 /v1/messages 免费开放且不计入月度额度；Plus {{PLUS_MONTHLY_PRICE_TEXT}}、{{PLUS_MONTHLY_REQUESTS}} 次，并额外开放完整 Plus 模型池。免费模型可能用于改进或试用目的，请避免提交敏感信息。", "Free includes {{FREE_MONTHLY_REQUESTS}} monthly requests and the <span data-live-free-count>live free model catalog</span>, including sponsored DeepSeek V4 Flash and DeepSeek V4 Pro. MiniMax M3 is free through /v1/messages and does not count against monthly quota. Plus is {{PLUS_MONTHLY_PRICE_TEXT}} with {{PLUS_MONTHLY_REQUESTS}} requests and the full Plus model pool. Avoid sensitive data on free models."],
      ...
      ["额外免费开放模型，需走 /v1/messages，且不计入月度额度，适合更长上下文和高质量输出场景。", "Additional free model available through /v1/messages and excluded from monthly quota accounting, suited to longer context and higher-quality outputs."],
      ...
      ["付费 Go 通道赞助开放，适合复杂分析、代码审阅、规划和更稳的推理流程。", "Sponsored through the paid Go route; suitable for complex analysis, code review, planning, and steadier reasoning workflows."],
      ...
      ["鉴权方式保持 Bearer API Key。Chat Completions 继续走 OpenAI-compatible；MiniMax M3 走 Anthropic-compatible /v1/messages。", "Authentication remains Bearer API Key. Chat Completions stay OpenAI-compatible, while MiniMax M3 uses Anthropic-compatible /v1/messages."],
      ...
      ["大多数模型兼容 OpenAI 风格的请求格式；MiniMax M3 例外，需要请求 /v1/messages 并使用 Anthropic Messages 格式。", "Most models are OpenAI-compatible, but MiniMax M3 is an exception and must use /v1/messages with the Anthropic Messages format."],
      ["Plus 价格和额度是什么？", "What are the Plus price and quota?"],
      ["Plus 为 {{PLUS_MONTHLY_PRICE_TEXT}}，包含 {{PLUS_MONTHLY_REQUESTS}} 次/月请求额度；Free 是 {{FREE_MONTHLY_REQUESTS}} 次/月，可调用实时同步的免费模型池、赞助开放的 DeepSeek V4 Flash / DeepSeek V4 Pro；MiniMax M3 额外免费开放且不计入月度额度。", "Plus is {{PLUS_MONTHLY_PRICE_TEXT}} with {{PLUS_MONTHLY_REQUESTS}} requests/month. Free includes {{FREE_MONTHLY_REQUESTS}} requests/month, the live free model catalog, sponsored DeepSeek V4 Flash / DeepSeek V4 Pro, and MiniMax M3 for free without monthly quota impact."],
      ["可以在同一个项目里切换模型吗？", "Can I switch models in one project?"],
      ["可以。多数模型继续通过 /v1/chat/completions 切换；MiniMax M3 例外，需要单独走 /v1/messages。", "Yes. Most models still switch through /v1/chat/completions, while MiniMax M3 is the exception and must use /v1/messages."],
